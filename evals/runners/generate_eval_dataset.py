...
# Allow the runner to work from a clean checkout before editable installation.
...
def generate_eval_dataset(
    ...
    out_jsonl_path: Path,
    model_identifier: str = "customer-support-assistant",
    mock_mode: bool = True,
...
def main():
    ...
    parser.add_argument(
        "--tests-json",
        default=str(PROJECT_ROOT / "evals" / "cases" / "support-cases.json"),
        help="Path to the test suite JSON file."
    )
    parser.add_argument(
        "--out-jsonl",
        default=str(PROJECT_ROOT / "evals" / "results" / "evaluation.jsonl"),
